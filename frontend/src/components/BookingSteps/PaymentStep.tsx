...
const PaymentStep = ({
  ...
  useEffect(() => {
    if (appointmentId && patientName) {
      console.log("🔥 Trigger Payment Now");
  ...
  // console.log("PaymentStep Render:", { appointmentId, patientName });
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      var options = {
        ...
        //   },
        // },
        name: "Doctor Consultation Platform", //your business name
        description: `Consultation with Dr. ${doctorName}`,
        order_id: orderId, // This is a sample Order ID. Pass the `id` obtained in the response of Step 1
        handler: async (response: any) => {
          try {
            const verifyPayment = await httpService.postWithAuth(
            ...
            if (verifyPayment.success) {
              ...
              setTimeout(() => {
                if (onPaymentSuccess) {
                  onPaymentSuccess(verifyPayment.data);
                } else {
                  onConfirm();
                }
              }, 3000); // 2 sec delay
              ...
              throw new Error(
                verifyPayment.message || "Payment verification failed",
              );
            }
          } catch (error: any) {
  ...
  const handlePaynow = async () => {
    ...
    if (!appointmentId) {
      ...
      await onConfirm(); // 👈 wait for booking
    } else {
      handlePayment();
    }
  };
        ...
        <div className="bg-gray-100 rounded-lg p-6 mb-8">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">₹{consultationFees}</span>
            </div>
